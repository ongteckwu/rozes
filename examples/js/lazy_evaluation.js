...
async function main() {
  console.log('⚡ Lazy Evaluation API Showcase\n');
  ...
  // Sample dataset
  const csv = `name,age,city,salary,department
...
Liam,40,Boston,110000,Engineering`;
  ...
  const eagerResult = df.select(['name', 'age']).head(5);
  ...
  try {
    ...
    const lazyDf = df.lazy();  // No execution yet
    const lazyWithOps = lazyDf
      .select(['name', 'age'])  // Add to query plan
      .limit(5);                // Add to query plan
    const lazyResult = lazyWithOps.collect();  // Execute now!
  ...
  try {
    const lazyOpt = df.lazy()
      .select(['name', 'salary'])
      .limit(3);

    console.log('Executing optimized query...');
    const result = lazyOpt.collect();
    result.show();
    console.log();
    result.free();
  } catch (err) {
  ...
  try {
    ...
    const lazyChained = df.lazy()
      .select(['name', 'department', 'salary'])
      .limit(8);

    console.log('Executing chained query...');
    const chainedResult = lazyChained.collect();
    chainedResult.show();
    console.log();
    chainedResult.free();
  } catch (err) {
  ...
  const eager = df.select(['name', 'age']).head(5);
  ...
  try {
    ...
    const lazy = df.lazy().select(['name', 'age']).limit(5);
    const lazyCollected = lazy.collect();
