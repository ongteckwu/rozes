/**
 * Advanced Aggregations API Showcase
 *
...
async function main() {
  console.log('📊 Advanced Aggregations API Showcase\n');
  ...
  const csv = `student,math,science,english,overall,grade
...
Jack,94,91,89,91.3,A`;
  ...
  const medianMath = df.median('math');
  const medianScience = df.median('science');
  const medianEnglish = df.median('english');
  ...
  const mathQ25 = df.quantile('math', 0.25);  // 25th percentile (Q1)
  const mathQ50 = df.quantile('math', 0.50);  // 50th percentile (median, Q2)
  const mathQ75 = df.quantile('math', 0.75);  // 75th percentile (Q3)
  ...
  const overallQ10 = df.quantile('overall', 0.10);  // 10th percentile
  const overallQ90 = df.quantile('overall', 0.90);  // 90th percentile
  ...
  const gradeCounts = df.valueCounts('grade');
  gradeCounts.show();
  console.log();

  // Calculate percentages
  console.log('Grade percentages:');
  const totalStudents = df.shape.rows;
  const gradeCountsData = gradeCounts.column('count');
  const grades = gradeCounts.column('grade');
  if (gradeCountsData && grades) {
    for (let i = 0; i < grades.data.length; i++) {
      const grade = grades.data[i];
      const count = gradeCountsData.data[i];
  ...
  const corrMatrix = df.corrMatrix(['math', 'science', 'english']);
  corrMatrix.show();
  console.log();
  ...
  function printStats(df, column) {
    const mean = df.mean(column);
    const median = df.median(column);
    const min = df.min(column);
    const max = df.max(column);
    const q25 = df.quantile(column, 0.25);
    const q75 = df.quantile(column, 0.75);
  ...
  function findOutliers(df, column) {
    const q1 = df.quantile(column, 0.25);
    const q3 = df.quantile(column, 0.75);
    const iqr = q3 - q1;
  ...
  const p90 = df.quantile('overall', 0.90);
  const p70 = df.quantile('overall', 0.70);
  const p50 = df.quantile('overall', 0.50);
  const p30 = df.quantile('overall', 0.30);
  ...
  const gradeA = df.filter((row) => row.get('grade') === 'A');
  const gradeB = df.filter((row) => row.get('grade') === 'B');
  const gradeC = df.filter((row) => row.get('grade') === 'C');
