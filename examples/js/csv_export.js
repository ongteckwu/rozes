...
async function main() {
  console.log('📊 CSV Export API Showcase\n');
  ...
  // Sample dataset
  const csv = `name,age,city,score,notes
...
Eve,32,Austin,93.2,"Product manager, runner"`;
  ...
  console.log('─'.repeat(50));
  const csv2 = df.toCSV({ includeHeaders: false });
  console.log(csv2);
  console.log();
  ...
  console.log('─'.repeat(50));
  const csvCRLF = df.toCSV({ lineEnding: '\r\n' });
  ...
  console.log('─'.repeat(50));
  const dfSubset = df.select(['name', 'age', 'score']);
  const csv6 = dfSubset.toCSV();
  console.log(csv6);
  console.log();
  ...
  console.log('─'.repeat(50));
  const dfFiltered = df.filter((row) => row.get('age') > 30);
  const csv7 = dfFiltered.toCSV();
  console.log(csv7);
  console.log();
  ...
  console.log('─'.repeat(50));
  const outputPath = join(__dirname, 'output_example.csv');
  ...
  console.log('─'.repeat(50));
  const originalCSV = df.toCSV();
  const dfReloaded = DataFrame.fromCSV(originalCSV);
  const reexportedCSV = dfReloaded.toCSV();
  ...
  console.log('─'.repeat(50));
  const specialCSV = `product,description,price
"Widget A","Contains comma, and ""quotes""",19.99
"Widget B","Line break:\nNewline inside",29.99
"Widget C","Tab:\tInside field",39.99`;

  const dfSpecial = DataFrame.fromCSV(specialCSV);
  const exportedSpecial = dfSpecial.toCSV();
