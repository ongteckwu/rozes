/**
 * String Operations API Showcase
 *
...
async function main() {
  console.log('📝 String Operations API Showcase\n');
  ...
  const csv = `name,email,product,description,tags
...
Eve Wilson,eve@example.org,Part E,"  Replacement part  ","#parts #maintenance"`;
  ...
  const dfLowerEmail = df.strLower('email');
  ...
  console.log('─'.repeat(50));
  const dfUpperProduct = df.strUpper('product');
  ...
  console.log('─'.repeat(50));
  const dfTrimProduct = df.strTrim('product');
  const dfTrimDesc = dfTrimProduct.strTrim('description');
  ...
  const dfContainsExample = df.strContains('email', 'example');
  dfContainsExample.select(['name', 'email', 'email_contains']).show();
  console.log();

  console.log('Find descriptions containing "widget":');
  const dfContainsWidget = df.strContains('description', 'widget');
  ...
  const dfReplaceProduct = df.strReplace('product', 'Widget', 'Component');
  dfReplaceProduct.select(['product']).show();
  console.log();

  console.log('Replace "#" with "Tag:" in tags:');
  const dfReplaceTags = df.strReplace('tags', '#', 'Tag:');
  ...
  const dfSliceName = df.strSlice('name', 0, 3);
  ...
  const dfEmailProcessed = df.strLower('email');
  ...
  const dfStartsWidget = df.strStartsWith('product', 'Widget');
  dfStartsWidget.select(['product', 'product_startswith']).show();
  console.log();

  console.log('Find tags starting with "#electronics":');
  const dfStartsElec = df.strStartsWith('tags', '#electronics');
  ...
  const dfComEmails = df.filter((row) => {
