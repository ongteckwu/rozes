...
async function main() {
  console.log('🔍 Missing Data API Showcase\n');
  ...
  const csv = `name,age,city,score,department
...
Grace,29,Seattle,89.5,Engineering`;
  ...
  const ageIsna = df.isna('age');
  ageIsna.show();
  console.log();

  console.log('Check score column for missing values:');
  const scoreIsna = df.isna('score');
  scoreIsna.show();
  console.log();
  ...
  const dfNoMissingAge = df.dropna('age');
  dfNoMissingAge.show();
  ...
  const dfNoMissingScore = df.dropna('score');
  ...
  const dfAgesFilled = df.fillna('age', 0);
  dfAgesFilled.show();
  console.log();

  console.log('Fill missing scores with mean (90.0):');
  const dfScoresFilled = df.fillna('score', 90.0);
  dfScoresFilled.show();
  console.log();

  console.log('Fill missing departments with "Unknown":');
  const dfDeptFilled = df.fillna('department', 'Unknown');
  dfDeptFilled.show();
  console.log();
  ...
  const dfStep1 = df.dropna('age');
  console.log('Step 1: After dropping missing age:');
  dfStep1.show();

  // Then fill remaining missing values
  const dfStep2 = dfStep1.fillna('city', 'Unknown City');
  const dfStep3 = dfStep2.fillna('score', 0.0);
  const dfFinal = dfStep3.fillna('department', 'Unknown');
  ...
  const deptNotna = df.notna('department');
  // Filter original df using the boolean column
  const hashedDf = df.filter((row) => {
  ...
  for (const col of columns) {
    const isnaResult = df.isna(col);
    const columnData = isnaResult.column(col + '_isna');
    if (columnData) {
      const values = columnData.data;
      let missingCount = 0;
