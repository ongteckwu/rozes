/**
 * Window Operations API Showcase (TypeScript)
 *
...
async function main(): Promise<void> {
  console.log('📈 Window Operations API Showcase (TypeScript)\n');
  ...
  const csv: string = `date,price,volume,high,low
...
2024-01-10,110.0,1500,112.0,109.0`;
  ...
  const dfRollingSum: DataFrameType = df.rollingSum('volume', 3);
  ...
  const dfSMA3: DataFrameType = df.rollingMean('price', 3);
  dfSMA3.select(['date', 'price', 'price_rolling_mean']).show();
  console.log();

  console.log('5-day moving average (SMA-5) of price:');
  const dfSMA5: DataFrameType = df.rollingMean('price', 5);
  ...
  const dfRollingMin: DataFrameType = df.rollingMin('price', 3);
  ...
  const dfRollingMax: DataFrameType = df.rollingMax('price', 3);
  ...
  const dfRollingStd: DataFrameType = df.rollingStd('price', 3);
  ...
  const dfCumSum: DataFrameType = df.expandingSum('volume');
  ...
  const dfCumMean: DataFrameType = df.expandingMean('price');
  ...
  const dfMultiSMA: DataFrameType = df
    .rollingMean('price', 3)
    .rollingMean('price', 5);

  // For display, select relevant columns
  console.log('(Showing first 8 rows for clarity)');
  const dfDisplay: DataFrameType = dfMultiSMA.select(['date', 'price', 'price_rolling_mean']);
  ...
  const dfBollinger: DataFrameType = df
  ...
  const dfVolumeMA: DataFrameType = df.rollingMean('volume', 3);
  ...
  const dfHighRolling: DataFrameType = df.rollingMax('high', 3);
  const dfLowRolling: DataFrameType = dfHighRolling.rollingMin('low', 3);
  ...
  const dfTrend: DataFrameType = df.expandingMean('price');
  ...
  const dfTechnical: DataFrameType = df
    .rollingMean('price', 3)
    .rollingStd('price', 3)
