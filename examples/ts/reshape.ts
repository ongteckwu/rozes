/**
 * Reshape Operations API Showcase (TypeScript)
 *
...
async function main(): Promise<void> {
  console.log('🔄 Reshape Operations API Showcase (TypeScript)\n');
  ...
  const salesLong: string = `store,product,quarter,sales
...
Store B,Gadget,Q2,95`;
  ...
  const dfLong: DataFrameType = DataFrame.fromCSV(salesLong);
  console.log('Long format (original):');
  dfLong.show();
  console.log();

  console.log('Pivot: Create product columns from rows');
  const dfPivot: DataFrameType = dfLong.pivot('store', 'product', 'sales', 'sum');
  ...
  const scoresWide: string = `student,math,science,english
Alice,95,92,88
Bob,78,85,82
Charlie,92,89,95`;

  const dfWide: DataFrameType = DataFrame.fromCSV(scoresWide);
  console.log('Wide format (original):');
  dfWide.show();
  console.log();

  console.log('Melt: Stack subject columns into rows');
  const dfMelted: DataFrameType = dfWide.melt(['student'], ['math', 'science', 'english'], 'subject', 'score');
  ...
  const quarterly: string = `metric,Q1,Q2,Q3,Q4
Revenue,100,120,115,130
Costs,60,65,62,70
Profit,40,55,53,60`;

  const dfQuarterly: DataFrameType = DataFrame.fromCSV(quarterly);
  console.log('Original (metrics as rows):');
  dfQuarterly.show();
  console.log();

  console.log('Transposed (quarters as rows):');
  const dfTransposed: DataFrameType = dfQuarterly.transpose();
  dfTransposed.show();
  console.log();
  ...
  const temperatures: string = `city,jan,feb,mar
NYC,32,35,45
LA,55,58,62
Chicago,25,28,38`;

  const dfTemp: DataFrameType = DataFrame.fromCSV(temperatures);
  console.log('Original (months as columns):');
  dfTemp.show();
  console.log();

  console.log('Stacked (months as rows):');
  const dfStacked: DataFrameType = dfTemp.stack();
  dfStacked.show();
  console.log();
  ...
  const inventory: string = `warehouse,product,quantity
East,WidgetA,100
East,WidgetB,150
West,WidgetA,120
West,WidgetB,130`;

  const dfInventory: DataFrameType = DataFrame.fromCSV(inventory);
  console.log('Original (stacked format):');
  dfInventory.show();
  console.log();

  console.log('Unstacked (products as columns):');
  const dfUnstacked: DataFrameType = dfInventory.unstack();
  dfUnstacked.show();
  console.log();
  ...
  const transactions: string = `customer,product,month,amount
...
Bob,Gadget,Feb,90`;
  ...
  const dfTrans: DataFrameType = DataFrame.fromCSV(transactions);
  console.log('Transaction data:');
  dfTrans.show();
  console.log();

  console.log('Pivot: Total sales by customer and product (sum):');
  const dfPivotSum: DataFrameType = dfTrans.pivot('customer', 'product', 'amount', 'sum');
  dfPivotSum.show();
  console.log();

  console.log('Pivot: Average sales by customer and product (mean):');
  const dfPivotMean: DataFrameType = dfTrans.pivot('customer', 'product', 'amount', 'mean');
  dfPivotMean.show();
  console.log();
  ...
  const studentData: string = `student,grade,math,science,english
Alice,A,95,92,88
Bob,B,78,85,82`;

  const dfStudents: DataFrameType = DataFrame.fromCSV(studentData);
  console.log('Original:');
  dfStudents.show();
  console.log();

  console.log('Melted (keep student and grade as IDs):');
  const dfStudentsMelted: DataFrameType = dfStudents.melt(
    ['student', 'grade'],
  ...
  const dfRoundTrip1: DataFrameType = dfWide.melt(['student'], ['math', 'science', 'english'], 'subject', 'score');
  dfRoundTrip1.show();
  console.log();

  console.log('Step 3: Pivot back to wide format');
  const dfRoundTrip2: DataFrameType = dfRoundTrip1.pivot('student', 'subject', 'score', 'sum');
  dfRoundTrip2.show();
  console.log();
  ...
  const survey: string = `age_group,gender,response
18-25,M,Yes
18-25,F,Yes
18-25,M,No
26-35,M,Yes
...
36-45,F,Yes`;
  ...
  const dfSurvey: DataFrameType = DataFrame.fromCSV(survey);
  ...
  const dfChained: DataFrameType = dfWide
