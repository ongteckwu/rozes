/**
 * Apache Arrow Interop API Showcase (TypeScript)
 *
...
import { Rozes, DataFrame as DataFrameType, ArrowSchema } from '../../dist/index.mjs';
...
async function main(): Promise<void> {
  console.log('🏹 Apache Arrow Interop API Showcase (TypeScript)\n');
  ...
  const csv: string = `name:String,age:Int64,score:Float64,active:Bool
Alice,30,95.5,true
Bob,25,87.3,false
Charlie,35,91.0,true`;
  ...
  try {
    const arrowSchema: ArrowSchema = df.toArrow();
  ...
  try {
    const dfFromArrow: DataFrameType = DataFrame.fromArrow(arrowSchema);
  ...
  try {
    const schema1: ArrowSchema = df.toArrow();
    console.log('Step 1: Exported schema');
    console.log(`  Fields: ${schema1.schema.fields.length}`);
    console.log();

    const dfRoundTrip: DataFrameType = DataFrame.fromArrow(schema1);
    console.log('Step 2: Re-imported DataFrame');
    dfRoundTrip.show();
    console.log();

    const schema2: ArrowSchema = dfRoundTrip.toArrow();
  ...
  for (const test of validSchemas) {
    ...
    try {
      const testDf: DataFrameType = DataFrame.fromArrow(test.schema);
